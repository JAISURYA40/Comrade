package com.comrade.android.services.quickTiles
...
class FocusQuickTileService : TileService() {
    private val TAG = "Comrade.FocusQuickTileService"
    ...
    override fun onStartListening() {
        try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.UPSIDE_DOWN_CAKE) {
                val uriString = if (isFocusActive) "com.comrade.android://open/activeSession"
                else "com.comrade.android://open/focus"
                tile?.activityLaunchForClick = AppUtils.getPendingIntentForComradeUri(this, uriString)
            }
        ...
        try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.UPSIDE_DOWN_CAKE) {
                startActivityAndCollapse(
                    AppUtils.getPendingIntentForComradeUri(this, uriString)
                )
            } else {
                startActivityAndCollapse(
                    AppUtils.getIntentForComradeUri(this, uriString)
                )
            }
        } catch (e: Exception) {
