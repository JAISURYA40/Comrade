...
object PermissionsHelper {
    private const val TAG = "Comrade.PermissionsHelper"
    ...
    fun getAndAskDisplayOverlayPermission(context: Context, askPermissionToo: Boolean): Boolean {
        ...
        if (askPermissionToo) {
            try {
                ...
                Toast.makeText(
                    context,
                    "Please allow Comrade to display overlay",
                    Toast.LENGTH_LONG
                ).show()
            } catch (e: Exception) {
                Log.e(
    ...
    fun getAndAskNotificationPermission(
        ...
        if (askPermissionToo) {
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.TIRAMISU) {
                NewActivitiesLaunchHelper.openComradeNotificationSection(activity)
            } else {
