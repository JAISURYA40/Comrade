 ...
 *
 */
package com.comrade.android.services.vpn
 ...
 */
class ComradeVpnService : VpnService() {
    companion object {
        private const val TAG = "Comrade.VpnService"
    }

    private val mBinder = ServiceBinder(this@ComradeVpnService)
        ...
        get() = Runnable {
            try {
                DatagramChannel.open().use { tunnel ->
                    check(this@ComradeVpnService.protect(tunnel.socket())) { "Cannot protect the vpn socket tunnel" }
                    val serverAddress: SocketAddress = InetSocketAddress("localhost", 0)
                    tunnel.connect(serverAddress)
                    tunnel.configureBlocking(false)

                    val builder = this@ComradeVpnService.Builder()
                    ...
                    synchronized(this@ComradeVpnService) {
                ...
                SharedPrefsHelper.insertCrashLogToPrefs(this@ComradeVpnService, e)
                stopAndDisposeService()
            } catch (e: IOException) {
    ...
    override fun onBind(intent: Intent): IBinder? {
        return if (intent.action == ServiceBinder.ACTION_BIND_TO_COMRADE) mBinder else null
    }
}
