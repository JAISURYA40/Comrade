...
/// A Riverpod state notifier provider that manages [ComradeSettings].
final comradeSettingsProvider =
    StateNotifierProvider<ComradeSettingsNotifier, ComradeSettings>(
  (ref) => ComradeSettingsNotifier(),
);

/// This class manages the state of comrade settings.
class ComradeSettingsNotifier extends StateNotifier<ComradeSettings> {
  ComradeSettingsNotifier() : super(defaultComradeSettingsModel) {
    ...
    if (addListenerToo) {
      ...
      Future.delayed(
        ...
        () => addListener(
          ...
          (state) => dao.saveComradeSettings(state),
        ),
      );
    }
  ...
  void updateAppVersion() => state = state.copyWith(
        appVersion: MethodChannelService.instance.deviceInfo.comradeVersion,
      );
}
